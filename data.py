...
dataset['Gender'] = dataset['Gender'].map({'F': 0, 'M': 1, 'T': 2}).astype(np.int64)
...
def aadhaar_ratio(state):
    df = pd.read_sql("select * from state_counts where State=\'{0}\'".format(state), conn)
    appl_ratio = (float(df['Aadhaar_Count'])/float(df['Population']))*100
    if appl_ratio <= 100:
        appl_sent = '''Aadhaar applications percentage of {1} is {0}%'''.format(str(round(appl_ratio, 2)), state)
    else:
        appl_sent = '''Aadhaar applications percentage of {0} is exceeded more than 100%'''.format(state)
    return appl_sent
...
def gender_ratio(df):
    ...
    gender_sent = '''{0}% men and {1}% women have applied for Aadhaar'''.format(str(m_ratio), str(f_ratio))
    return gender_sent


def contact_details(df):
    yes_df = df[df['Contact Available'] == 1]['Contact Available'].count()
    contact_sent = '''{0} of state population have provided contact details'''.format(str(Fraction(yes_df, len(df))))
    return contact_sent
...
def approval_count(df):
    ...
    approval_sent = '''{0}% of Aadhaars were approved by UIDIA'''.format(str(round(ratio, 2)))
