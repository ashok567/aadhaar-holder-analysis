...
def aadhaar_ratio(state):
    ...
    if appl_ratio <= 100:
        appl_sent = '''Aadhaar applications percentage is {0}%'''.format(str(round(appl_ratio, 2)))
    else:
        appl_sent = '''Aadhaar applications percentage is exceeded more than 100%'''
    return appl_sent
...
def contact_details(df):
    ...
    contact_sent = '''{0} of population have provided contact details'''.format(str(Fraction(yes_df, len(df))))
    return contact_sent
...
def approval_count(df):
    ...
    approval_sent = '''{0}% of aadhaar were approved by UIDIA'''.format(str(round(ratio, 2)))
