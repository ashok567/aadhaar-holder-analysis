...
def aadhaar_ratio(state):
    ...
    if appl_ratio <= 100:
        appl_sent = '''Aadhaar applications percentage of {1} \
            is {0}%'''.format(str(round(appl_ratio, 2)), state)
    else:
        appl_sent = '''Aadhaar applications percentage of {0} is exceeded more \
            than 100%'''.format(state)
    return appl_sent
...
def gender_ratio(df):
    ...
    gender_sent = '''{0}% men and {1}% women have applied for \
        Aadhaar'''.format(str(m_ratio), str(f_ratio))
    return gender_sent


def contact_details(df):
    yes_df = df[df['Contact Available'] == 1]['Contact Available'].count()
    contact_sent = '''{0} of state population have provided contact \
        details'''.format(str(Fraction(yes_df, len(df))))
    return contact_sent
